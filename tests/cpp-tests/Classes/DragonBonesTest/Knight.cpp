...
bool KnightGame::init()
{
    ...
    text->setPosition(center.width / 2, 60.f);
...
void KnightGame::onExit()
{
    LayerColor::onExit();
    cocos2d::Director::getInstance()->getScheduler()->unschedule(schedule_selector(KnightGame::_enterFrameHandler), this);
    ...
    _armArmature(nullptr)
{
    ...
    const auto center = cocos2d::Director::getInstance()->getVisibleSize();
    _armatureDisplay->setPosition(center.width * 0.5f, KnightGame::GROUND);
