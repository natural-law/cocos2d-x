...
const float CoreElementGame::G = -1.2f;
CoreElementGame* CoreElementGame::instance = nullptr;

CoreElementGame::~CoreElementGame()
{
    CC_SAFE_DELETE(_player);
    
    // clean the bullets
    int i = (int) _bullets.size();
    while (i--)
    {
...
bool CoreElementGame::init()
{
    ...
    if (dragonBonesData)
    {
        ...
        text->setPosition(visibleSize.width / 2, 60.f);
...
void CoreElementGame::onExit()
{
    LayerColor::onExit();
    cocos2d::Director::getInstance()->getScheduler()->unschedule(schedule_selector(CoreElementGame::_enterFrameHandler), this);
    ...
    _target()
{
    ...
    const auto visibleSize = cocos2d::Director::getInstance()->getVisibleSize();
    _armatureDisplay->setPosition(visibleSize.width / 2, CoreElementGame::GROUND);
...
void Mecha::_updatePosition()
{
    ...
    if (_speedX != 0.f)
    {
        ...
        {
            _armatureDisplay->setPosition(visibleSize.width, position.y);
        }
    }
...
bool CoreElementBullet::update()
{
    ...
    const auto visibleSize = cocos2d::Director::getInstance()->getVisibleSize();
    if (
        position.x < -100.f || position.x >= visibleSize.width + 100.f ||
        position.y < -100.f || position.y >= visibleSize.height + 100.f
        )
    {
        dragonBones::WorldClock::clock.remove(_armature);
